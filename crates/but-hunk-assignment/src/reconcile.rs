...
impl HunkAssignment {
    fn set_from(
        ...
        // Override branch_ref_bytes based on whether the current assignment already has one
        ...
        // Validate: if branch_ref_bytes is set, ensure it exists in the workspace.
...
pub(crate) fn assignments(
    ...
    // Precompute a set of all valid branch refs for O(1) validation
    ...
    for new_assignment in new {
        ...
        match intersecting.len().cmp(&1) {
            ...
            Ordering::Greater => {
                ...
                // If requested, reset to unassigned on multiple overlapping with different targets
