    ...
    if [ "$NEW_BRANCH" != "gitbutler/workspace" ]; then
        ...
        cleanup_hook() {
            ...
            if [ -f "$HOOKS_DIR/${_hook}-user" ]; then
                # Only restore if the current hook is still ours — if someone
                # (user or external manager) rewrote it after --force-hooks,
                # the backup is stale and restoring it would lose their changes.
                if grep -q "GITBUTLER_MANAGED_HOOK_V1" "$HOOKS_DIR/${_hook}" 2>/dev/null; then
                    ...
                    echo "  Skipped: ${_hook} was modified since installation, leaving as-is"
                fi
