...
///
/// Skips auth for:
/// - Requests with a localhost `Origin` header (local browser — trusted because
///   browsers cannot spoof `Origin` in JS, and in tunnel mode it distinguishes
///   a local browser from one accessing via the tunnel URL)
...
/// - When remote access is not configured (`auth_state` is `None`)
pub async fn auth_middleware(
    ...
    let auth = match auth {
        Some(auth) => auth,
        // No remote origin configured — server is localhost-only, no auth needed.
        None => return next.run(req).await,
    };

    // Requests from a localhost Origin are trusted: the local browser is on the
    // same machine as the server. In tunnel mode the tunnel origin is different,
    // so a remote browser cannot spoof this without being blocked by
    // localhost_only_middleware first.
    if let Some(origin) = req.headers().get(axum::http::header::ORIGIN)
        && crate::is_localhost_origin(origin.as_bytes())
    {
        return next.run(req).await;
    }
