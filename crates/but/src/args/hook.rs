...
pub enum Subcommands {
    ...
    /// Install GitButler-managed git hooks (pre-commit, post-checkout, pre-push).
    ///
    /// Adds GitButler's workspace guard and lifecycle hooks to the repository's
    /// hooks directory (or `core.hooksPath` if set). Existing user hooks are
    /// backed up to `<hook>.gitbutler-backup` before being replaced.
    ///
    /// If an external hook manager (prek, lefthook, husky, pre-commit) is
    /// detected, GitButler will not overwrite its hooks; instead it will print
    /// integration instructions for wiring `but hook` subcommands into the
    ...
    /// ```text
    /// but hook install
    /// but hook install --force
    /// but hook install --format json
    /// ```
    ///
    #[clap(verbatim_doc_comment)]
    Install {
        /// Re-enable installation even if previously disabled by config or an
        /// external hook manager is detected. Persists `gitbutler.installHooks=true`.
        #[clap(long)]
        force: bool,
    },

    /// Remove GitButler-managed git hooks.
    ///
    /// Best-effort cleanup that signature-checks each managed hook before
    /// touching it, so user or external hook-manager hooks are left intact.
    /// Restores any pre-existing user hook from its `<hook>.gitbutler-backup`
    /// companion file.
    ///
    /// ## Examples
    ///
    /// ```text
    /// but hook uninstall
    /// but hook uninstall --format json
    /// ```
    ///
    #[clap(verbatim_doc_comment)]
    Uninstall,
